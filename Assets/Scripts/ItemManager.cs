...
public class ItemManager : MonoBehaviour
{
    ...
    [Tooltip("아이템 획득시 띄울 이미지를 할당")]
    public Sprite collectedItemSprite;  // TempItemImage_0 Sprite
    private bool IsActive = false;
    private UnityEvent script; // 외부에서 할당 함
    void Awake()
    {
    ...
    public void ItemGet(UnityEvent script, bool IsAutoUsing = false)
    {
        if (IsAutoUsing) { script.Invoke(); return; }
        this.script = script;
        IsActive = true;
        itemImgUI.sprite = collectedItemSprite;  // 이미지 할당
